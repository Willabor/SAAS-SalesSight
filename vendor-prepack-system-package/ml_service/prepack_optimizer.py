...
Purpose: Demonstrate prepack optimization algorithm for planning docs

🎨 CRITICAL MISSING FEATURE: COLOR-AWARENESS NOT IMPLEMENTED!

This draft code does NOT properly handle color-specific prepacks!
- Prepacks are color-specific (one color per box)
- Must optimize PER COLOR first, then aggregate
- Current code optimizes by size only - WRONG APPROACH!

Before production use, this MUST be refactored to:
1. Group needs by COLOR first
2. For each COLOR, run optimization to determine best pack
3. Calculate how many boxes of each [Color + Pack] combination
4. Return recommendations like: "5 boxes Pack A (Black) + 2 boxes Pack A (Olive)"

See updated planning docs for correct color-aware algorithm design.

================================================================================

Prepack Optimization Module
